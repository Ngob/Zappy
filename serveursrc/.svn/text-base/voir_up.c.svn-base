/*
** voir_up.c for  in /home/capot_r//tek2/c/zappy/serveursrc
** 
** Made by romain capot
** Login   <capot_r@epitech.net>
** 
** Started on  Sun Jul 10 13:30:17 2011 romain capot
** Last update Sun Jul 10 13:30:18 2011 romain capot
*/
...
int		setGood_xy(int y, int limite)
{
  if (y >= limite)
    y %= limite;
  if (y < 0)
...
void	        print_voir_to_client(t_data *d, int fd, char *res)
{
  res = (res == NULL) ? (str_fmt("%c%s%c\n", '{', " ", '}')) :
    (str_fmt("%c%s%c\n", '{', res, '}'));
  printcs(d, fd, res);
  free(res);
}
...
void		voir_up(t_data *d, int fd, int pos_y)
{
  char		*res;
  int		i;
  int		vision;
